...
# Section Day/Time: Section 002 Wednesday 6pm-7pm
...
def task0():
    print ("Kevin Rothstein, my expected score is 60")
...
def name_counts(names):
    names_dict = {}
    for word in names:
        if word not in names_dict:
            names_dict[word] =1
        else:
            names_dict[word] += 1

    lst = names_dict.items()
    sortlst = sorted(lst, key = lambda word: word[0])
    answer = sorted(sortlst, key = lambda word: word[1], reverse= True)
    return answer
...
class House(object):
    ...
    def determine_size(self):
        ...
        if self.house_color == 'red':
            return "small"
        if self.house_color == 'purple':
            return "medium"
        if self.house_color == 'brick':
            return "medium"
